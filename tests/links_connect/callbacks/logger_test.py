...
def mock_chained(mocker) -> Tuple[Mock, Mock]:
    ...
    mock_on_recv = mocker.patch(
        "links_connect.callbacks.ChainableCallback.on_recv", autospec=True
    )
    mock_on_sent = mocker.patch(
        "links_connect.callbacks.ChainableCallback.on_sent", autospec=True
    )
...
def test_logger(mock_chained):
    ...
    clbk = LoggerCallback(name="LoggerCallbackFirst") + LoggerCallback(
        name="LoggerCallbackSecond"
    )
